...
- vcftools.
<details>
<summary>Установленные библиотеки</> /
    autoconf 
    automake 
    make 
    gcc
...
"export DEBIAN_FRONTEND=noninteractive" выполнено локально в слое, поскольку иначе переменная окружения будет сохранена в докер контейнере, что нам может помешать в последствии.